...
void WaypointVisualizer::download_waypoints() {  // put all data in vectors
    ...
    if (!csvFile_waypoints.is_open()) {
        RCLCPP_ERROR(this->get_logger(), "Cannot open waypoint CSV: %s", waypoints_path.c_str());
        return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Loaded %zu waypoints from %s", waypoints.X.size(), waypoints_path.c_str());
}
